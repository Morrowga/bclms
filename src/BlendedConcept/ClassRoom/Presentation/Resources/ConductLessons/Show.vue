...
onBeforeUnmount(() => {
    ...
    if (storedData) {
        ...
        console.log('that is storage data ' + storageData.timeSpend + ',' + storageData.completed + ',' + storageData.progress + ',' + storageData.accuracy);
    }

    const storedCookie = getCookie('storybook');
    if (storedCookie) {
        const parsedData = JSON.parse(storedCookie);
        console.log('that is cookie data ' + parsedData.timeSpend + ',' + parsedData.completed + ',' + parsedData.progress + ',' + parsedData.accuracy);
    }
});
