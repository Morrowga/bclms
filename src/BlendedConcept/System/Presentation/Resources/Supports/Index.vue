                ...
                <VCol cols="12">
                    <section>
                        <VCard>
                            ...
                            <VDivider />
                            <vue-good-table class="role-data-table" styleClass="vgt-table" @column-filter="onColumnFilter"
                                :totalRows="props.technicalSupportList.meta.total" :selected-rows-change="selectionChanged"
                                :pagination-options="options" :rows="props.technicalSupportList.data" :columns="columns"
                                v-on:row-click="onRowClick" :select-options="{
                                    enabled: true,
                                ...
                                <template #table-row="dataProps">
                                    <div v-if="dataProps.column.field == 'name'">
                                        <div class="d-flex flex-row gap-2">
                                            <span>
                                                {{ dataProps.row.user.first_name }} {{ dataProps.row.user.last_name }}
                                            </span>
                                        </div>
                                    </div>
                                    <div v-if="dataProps.column.field == 'request'" style="max-width: 40%;">
                                        <span class="text-wrap">{{ dataProps.row.question }}</span>
                                    </div>
                                    <div v-if="dataProps.column.field == 'date'">
                                        <span class="text-no-wrap">{{ dataProps.row.date }}</span>
                                    </div>
                                    <div v-if="dataProps.column.field == 'status'">
                                        <VChip color="success" v-if="!dataProps.row.has_responded">Active</VChip>
                                        <VChip color="gray" v-if="dataProps.row.has_responded">Closed</VChip>
                                    </div>
                                    <div v-if="dataProps.column.field == 'action'">
                                        <VBtn variant="text" class="text-secondary disablePopUp"
                                            @click="deleteSupport(dataProps.row.id)">
                                            <VIcon icon="mdi-trash-can-outline" size="21" class="disablePopUp" />
                                        </VBtn>
                                    </div>
                                ...
                                <template #pagination-bottom>
                                    <VRow class="pa-4">
                                        <VCol cols="12" class="d-flex justify-space-between">
                                            <span>Showing
                                                {{ props.technicalSupportList.meta.from }} to
                                                {{ props.technicalSupportList.meta.to }} of
                                                {{ props.technicalSupportList.meta.total }}
                                                entries</span>
                                            <div>
                                                <div class="d-flex align-center">
                                                    <span class="me-2">Show</span>
                                                    <VSelect v-model="serverPerPage" density="compact"
                                                        :items="options.perPageDropdown"></VSelect>
                                                    <VPagination v-model="serverPage" size="small" :total-visible="5"
                                                        :length="props.technicalSupportList.meta
                                                            .last_page
                                                            " @next="onPageChange" @prev="onPageChange"
                                                        @click="onPageChange" />
                                                </div>
                                            </div>
                                        </VCol>
