...
for (let i = 0; i < props.teachers.data.length; i++) {
  org_teacher_array.value.push({
    ...
    name:
      props.teachers.data[i].first_name +
      " " +
      props.teachers.data[i].last_name,
  });
}
...
for (let i = 0; i < props.b2cUsers.data.length; i++) {
  b2c_users_array.value.push({
    ...
    name:
      props.b2cUsers.data[i].first_name +
      " " +
      props.b2cUsers.data[i].last_name,
  });
}
...
for (let i = 0; i < props.bcStaff.data.length; i++) {
  bc_staff_array.value.push({
    ...
    name:
      props.bcStaff.data[i].first_name + " " + props.bcStaff.data[i].last_name,
  });
}
...
let onFormSubmit = () => {
  ...
  let toArray = [];
  organization_user_ids.value.length > 0
    ? toArray.push("Organizations Admins")
    : "";
...
let getTeacherByOrganization = (id) => {
  axios.get(route("announcements.getb2bteachersbyorg", id)).then((res) => {
      ...
      <VForm class="mt-6" ref="refForm" @submit.prevent="onFormSubmit">
        <VRow justify="space-around" :gutter="10">
          ...
          <VCol cols="8">
            <VRow justify="start">
              <VCol cols="6">
                <VLabel class="tiggie-label required">Title</VLabel>
                <VTextField
                  v-model="form.title"
              ...
              <VCol cols="12">
                <VLabel class="tiggie-label required">Message</VLabel>
              ...
              <VCol cols="6">
                <VLabel class="tiggie-label required">Announcement By</VLabel>
                <VSelect
                  v-model="form.by"
                  class="blue-outline-field"
                  variant="plain"
              ...
              <VCol cols="6" :hidden="visibleToSelectBox">
                <VLabel class="tiggie-label required">Announcement to</VLabel>
                <VSelect
                  :ref="toSelect"
                  class="blue-outline-field"
                  variant="plain"
              ...
              <VCol cols="6" :hidden="visibleToBcStaffSelectBox"> </VCol>
              <VCol cols="6" :hidden="visibleToBcStaffSelectBox">
                <VLabel class="tiggie-label required"
                  >Announcement to BC Staff</VLabel
                >
                <MultiSelectBox
                  :items="bc_staff_array"
                  v-model="bc_staff_ids"
                />
              </VCol>
              <VCol cols="6" :hidden="visibleToOrganizationSelectBox"> </VCol>
              <VCol cols="6" :hidden="visibleToOrganizationSelectBox">
                ...
                <MultiSelectBox
                  :items="org_array"
                  v-model="organization_user_ids"
                />
              </VCol>
              <VCol cols="6" :hidden="visibleToTeacherSelectBox"> </VCol>
              <VCol cols="6" :hidden="visibleToTeacherSelectBox">
                <VLabel class="tiggie-label required"
                  >Announcement to Organisations Teachers</VLabel
                >
                <MultiSelectBox
                  :items="org_teacher_array"
                  v-model="b2b_teacher_ids"
                />
              </VCol>
              <VCol cols="6" :hidden="visibleToB2CUserSelectBox"> </VCol>
              <VCol cols="6" :hidden="visibleToB2CUserSelectBox">
                <VLabel class="tiggie-label required"
                  >Announcement to B2C User[s]</VLabel
                >
                <MultiSelectBox
                  :items="b2c_users_array"
                  v-model="b2c_user_ids"
                />
              </VCol>
              <VCol cols="6" :hidden="visibleToB2bTeacherList">
                ...
                >
                <MultiSelectBox
                  :items="b2bteacherbyorg"
                  v-model="b2bteacherbyorg_ids"
                />
              </VCol>
            </VRow>
          </VCol>
          <VCol cols="12" class="d-flex flex-wrap justify-center gap-10 mt-10">
            <Link :href="route('announcements.index')" class="text-black">
              <VBtn color="gray" height="50" class="" width="250">
                Cancel
              </VBtn>
            </Link>
            <VBtn type="submit" class="" height="50" width="250"> Post </VBtn>
          </VCol>
        </VRow>
      </VForm>
    </VContainer>
