...
const maxTeacher = (organization) => {
  return organization?.subscription?.b2b_subscription?.num_teacher_license ?? 0;
};
const maxStudent = (organization) => {
  return organization?.subscription?.b2b_subscription?.num_student_license ?? 0;
};
          ...
          <div class="app-user-search-filter d-flex align-center gap-6">
            ...
            <Link :href="route('organizations.create')">
              <VBtn height="40" density="compact">
                <span class="text-capatlize text-white"> Add New </span>
              </VBtn>
            </Link>
          </div>
        </VCardText>
        ...
        >
          <template #table-row="props">
            <div v-if="props.column.field == 'name'">
              <Link :href="route('organizations.show', props.row.id)">
            ...
            <div v-if="props.column.field == 'status'">
              <VChip color="success" class="v-chip"> Active </VChip>
            </div>
                ...
                <VList v-if="user_role == 'BC Staff'">
                  <VListItem
                    @click="
                      () =>
                        router.get(
                          route('organizations.addSubscription', props.row.id)
                        )
                    "
                  >
                    <VListItemTitle>Add Subscription</VListItemTitle>
                  </VListItem>
                </VList>
                <VList v-else>
                  <VListItem
                    @click="
                      () =>
                        router.get(route('organizations.edit', props.row.id))
                    "
                  >
