...
Route::group(['middleware' => ['auth', 'isSuperAdmin']], function () {
    ...
    Route::any(config('pagebuilder.general.assets_url') . '{any}', [DashBoardController::class, 'getAssertUrl'])->where('any', '.*');
    ...
    if (config('pagebuilder.router.use_router')) {
        ...
        Route::any('/bc/{any}', [DashBoardController::class, 'UseRouter'])->where('any', '.*')->withoutMiddleware(['auth', 'isSuperAdmin']);
    }
});

// handle requests to retrieve uploaded file
Route::any(config('pagebuilder.general.uploads_url') . '{any}', [DashBoardController::class, 'uploadsUrl'])->where('any', '.*');
